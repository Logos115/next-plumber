...
export default function StockInPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setMessage({
          type: "err",
          text: typeof data === "string" ? data : (data?.message ?? "Failed to record stock-in"),
        });
        return;
      }
      ...
      setMessage({
        type: "ok",
        text: `Recorded ${data.transaction?.quantity ?? quantity} ${String(unit).toLowerCase()} for ${data.itemName ?? "item"}. New stock: ${data.newStock ?? "—"}.`,
      });
      setQuantity("");
      setDeliveryReference("");
    } catch {
      setMessage({ type: "err", text: "Could not record stock-in." });
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <main>
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Stock In</h1>
        <p className="mt-1 text-sm text-slate-500">Log deliveries to increase stock</p>
      </div>
        ...
        <form onSubmit={submit} className="flex flex-col gap-4 max-w-md">
          ...
          <label className="flex flex-col gap-1">
            <span className="text-xs font-medium text-slate-500">
              Supplier / delivery reference <span className="text-slate-400">(optional)</span>
            </span>
            <input
              type="text"
              placeholder="e.g. PO-12345, Docket #789"
